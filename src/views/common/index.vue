<template>
    <div class="common_content">
        <el-table :data="tableData" style="width: 100%" border stripe :show-overflow-tooltip="true" class="content_table">
            ...
            <el-table-column align="center" prop="Area" label="Area" min-width="100">
            </el-table-column>
            ...
            <el-table-column align="center" prop="Cached" label="Cached" min-width="120">
            </el-table-column>
            ...
            <el-table-column align="center" fixed="right" label="IsOpened" min-width="100">
                <template slot-scope="scope" class="options">
                    <div class="options">
                        ...
                        <a @click="changeState(scope.row)" v-if="!scope.row.IsOpened">open
                        </a>
                    </div>
                </template>
            </el-table-column>
            ...
            <el-dialog title="MaxCache-Edit" :visible.sync="common_dialog">
                <el-form :model="reqdata">
                    <el-form-item label="MaxCache">
                        <el-input-number v-model="reqdata.MaxCache" controls-position="right" :max="60000" :min="1"></el-input-number>
                    </el-form-item>
                </el-form>
        ...
        methods: {
            ...
            changeState(row) {
                this.$confirm("请确认是否打开", "提示", {
                    ...
                    this.$http.get(`${api.commonOpen}?serverid=${row.ID}`).then(res => {
                        if (res && res.Code === 1) {
                            this.getData();
                        }
                    });
